...
public class BenchmarkTest extends MyTestCase {
	...
	protected StatisticalReport runGame(final String configFiles[],
			final String extraConfigFiles[]) {

		final ParameterDatabase root = new ParameterDatabase();
		ParameterDatabase pdb = null;
		try {
			for (final String configFile : configFiles) {
				pdb = new ParameterDatabase(new File(path, configFile));
				root.addParent(pdb);
			}
			for (final String extraConfigFile : extraConfigFiles) {
				pdb = new ParameterDatabase(new File(path, extraConfigFile));
				root.addParent(pdb);
			}
	...
	protected void checkEfficiency(final String marketId,
			final String configFiles[], final String extraConfigFiles[],
			final double meanLowerBound, final double meanUpperBound,
			final double stdevLowerBound, final double stdevUpperBound) {
	...
	protected void checkEfficiency(final String marketId,
			final String configFiles[], final String extraConfigFiles[],
			final double meanLowerBound, final double meanUpperBound,
			final double errorOfMean, final double stdevLowerBound,
			final double stdevUpperBound, final double errorOfStdev) {
