...
public class Shout implements Comparable<Shout>, Cloneable, Serializable {
	 ...
	 * 
	 * @param itor
	 * @return the cumulative volume of goods in the given shouts
	 */
	public static int totalVolume(Iterator<Shout> itor) {
		int volume = 0;
		while (itor.hasNext()) {
			Shout shout = itor.next();
			volume += shout.getQuantity();
		}
	...
	public static double maxPrice(final Shout s1, final Shout s2) {
		return Math.max(Shout.price(s1, Double.NEGATIVE_INFINITY),
				Shout.price(s2, Double.NEGATIVE_INFINITY));
	}
	...
	public static double minPrice(final Shout s1, final Shout s2) {
		return Math.min(Shout.price(s1, Double.POSITIVE_INFINITY),
				Shout.price(s2, Double.POSITIVE_INFINITY));
	}
