...
public class MarketMetricsReport extends DirectRevelationReport {
	...
	protected void calculateIndividually() {
		...
		for (final Specialist specialist2 : specialists) {
			...
			final EquilibriumCalculator equilCal = equilCals.get(specialist2.getId());
	...
	protected void calculateGlobally() {
		...
		final EfficiencyCalculator effCal = new EfficiencyCalculator(globalEquilCal,
				globalTransactions, traders);

		reportVariables(GameReport.GLOBAL, globalEquilCal, effCal);
	}
	...
	private void reportVariables(final String name,
		...
		board.reportValue(name + ReportVariable.SEPARATOR + GameReport.EQUILIBRIUM
				...
				equilCal.getEquilibriumQuantity());
		board.reportValue(name + ReportVariable.SEPARATOR + GameReport.EQUILIBRIUM
				+ ReportVariable.SEPARATOR + GameReport.PRICE,
				equilCal.getMidEquilibriumPrice());
		board.reportValue(name + ReportVariable.SEPARATOR + GameReport.EQUILIBRIUM
				+ ReportVariable.SEPARATOR + GameReport.PROFIT,
				effCal.getTheoreticalProfit());
