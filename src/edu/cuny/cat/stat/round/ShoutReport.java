...
public class ShoutReport implements GameReport {
	...
	public void eventOccurred(final AuctionEvent event) {
		if (event instanceof RoundStatPassEvent) {
			if (((RoundStatPassEvent) event).getPass() == RoundStatPassEvent.FIRST_PASS) {
				generateStat();
			}
		}
	}
				...
				if ((shout2.getState() == Shout.PLACED)
					...
					if (shout2.getState() == Shout.MATCHED) {
						if (shout2.isAsk()) {
							...
							pricesOfAskAccepted[index].newData(shout2.getPrice(),
									shout2.getQuantity());
						} else {
							bidAcceptedNums[index]++;
							pricesOfBidAccepted[index].newData(shout2.getPrice(),
									shout2.getQuantity());
						}
					}
				}
	...
	/**
	 * creates an array of {@link edu.cuny.math.dist.CumulativeDistribution}s.
	 * 
	 * @param num
