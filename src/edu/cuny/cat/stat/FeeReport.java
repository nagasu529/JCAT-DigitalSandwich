...
public class FeeReport implements GameReport {
	...
	protected LoggingController logController;

	public FeeReport() {
		logController = Galaxy.getInstance().getDefaultTyped(
	...
	public void eventOccurred(final AuctionEvent event) {
		if (event instanceof FeesAnnouncedEvent) {
			...
			board.reportValue(specialist.getId() + ReportVariable.SEPARATOR
					+ GameReport.FEE + ReportVariable.SEPARATOR
					+ ChargingPolicy.P_FEES[ChargingPolicy.INFORMATION_INDEX],
					specialist.getInformationFee());
			board.reportValue(specialist.getId() + ReportVariable.SEPARATOR
					+ GameReport.FEE + ReportVariable.SEPARATOR
					+ ChargingPolicy.P_FEES[ChargingPolicy.SHOUT_INDEX],
					specialist.getShoutFee());
			board.reportValue(specialist.getId() + ReportVariable.SEPARATOR
					+ GameReport.FEE + ReportVariable.SEPARATOR
					+ ChargingPolicy.P_FEES[ChargingPolicy.TRANSACTION_INDEX],
					specialist.getTransactionFee());
			board.reportValue(specialist.getId() + ReportVariable.SEPARATOR
					+ GameReport.FEE + ReportVariable.SEPARATOR
					+ ChargingPolicy.P_FEES[ChargingPolicy.PROFIT_INDEX],
					specialist.getProfitFee());
		} else if (event instanceof DayStatPassEvent) {
			switch (((DayStatPassEvent) event).getPass()) {
			case DayStatPassEvent.SECOND_PASS:
				if (logController.isLogging()) {
					printFees();
				}
			}
		}
