...
import edu.cuny.cat.stat.RoundlyReportVariableWriterReport;
...
import edu.cuny.stat.LoggingController;
import edu.cuny.stat.ReportVariableBoard;
 ...
 * <td valign=top>(whether to allow log globally, used by some reports only)</td>
 * </tr>
 * 
 * </table>
 * 
...
public final class GameController extends Observable implements
	...
	RoundlyReportVariableWriterReport roundlyReport;
	...
	public void setup(final ParameterDatabase parameters, final Parameter base) {
		...
		boolean logging = parameters.getBoolean(
				base.push(GameController.P_LOGGING), null, true);
		Galaxy.getInstance().putDefault(LoggingController.class,
				LoggingController.createLoggingController(logging));
		...
		registry = parameters.getInstanceForParameter(
				base.push(GameController.P_REGISTRY), null, Registry.class);
		if (registry instanceof Parameterizable) {
			((Parameterizable) registry).setup(parameters,
					base.push(GameController.P_REGISTRY));
		}
		GameController.logger.info(Utils.indent(registry.toString()));

		// this needs to be after setup of registry
		identityController.setup(parameters,
				base.push(ClientIdentityController.P_DEF_BASE));

		report = parameters.getInstanceForParameter(
				base.push(GameController.P_REPORT), null, GameReport.class);
		if (report instanceof Parameterizable) {
			((Parameterizable) report).setup(parameters,
					base.push(GameController.P_REPORT));
		}
		GameController.logger.info(Utils.indent(report.toString()));

		// TODO: temporarily put here to check if necessary to do roundly stat
		roundlyReport = getReport(RoundlyReportVariableWriterReport.class);

		console = parameters.getInstanceForParameter(
				base.push(GameController.P_CONSOLE), null, Console.class);
		if (console instanceof Parameterizable) {
			((Parameterizable) console).setup(parameters,
					base.push(GameController.P_CONSOLE));
		}
		GameController.logger.info(Utils.indent(console.toString()));

		clockController = parameters.getInstanceForParameter(
				base.push(ClockController.P_DEF_BASE), null, ClockController.class);
		if (clockController instanceof Parameterizable) {
			((Parameterizable) clockController).setup(parameters,
					base.push(ClockController.P_DEF_BASE));
		}
	...
	public void processEventInsideServer(final AuctionEvent event) {
		...
		if (event instanceof GameStartingEvent) {
			...
			if (roundlyReport != null) {
				// to send stat events to game reports after a round closes.
				RoundStatPassEvent rspEvent = null;
				for (int i = RoundStatPassEvent.START_PASS; i <= RoundStatPassEvent.END_PASS; i++) {
					rspEvent = new RoundStatPassEvent(i);
					rspEvent.setTime(event.getTime());
					notifyInsideListener(report, rspEvent);
					notifyInsideListener(console, rspEvent);
				}
			}
