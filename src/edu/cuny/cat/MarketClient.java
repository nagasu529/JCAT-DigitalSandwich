...
import edu.cuny.event.RichEventListener;
...
public class MarketClient extends GameClient implements RichEventListener {
	...
	public void setup(final ParameterDatabase parameters, final Parameter base) {
		...
		try {
			auctioneer = parameters.getInstanceForParameter(
					base.push(MarketClient.P_AUCTIONEER),
					defBase.push(MarketClient.P_AUCTIONEER), Auctioneer.class);
		} catch (final ParamClassLoadException e) {
			auctioneer = new GenericDoubleAuctioneer();
		}

		try {
			((Parameterizable) auctioneer).setup(parameters,
					base.push(MarketClient.P_AUCTIONEER));
	...
	protected class RegisterSession extends CatpReactiveSession {
		...
		public void processRequest(final CatpRequest request) throws CatException {
			...
			addEventDispatchingTask(new RegistrationEvent(traderId,
					auctioneer.getName()));
	...
	protected class SubscribeFromClientSession extends CatpReactiveSession {
		...
		public void processRequest(final CatpRequest request) throws CatException {
			...
			addEventDispatchingTask(new SubscriptionEvent(traderId,
					auctioneer.getName()));
			...
			if ((shoutId == null) || (shoutId.length() == 0)) {
				...
				if (currentShout != null) {
					if (currentShout.getState() == Shout.PENDING) {
						...
						shout = new Shout(shoutId, price,
								reqType.equalsIgnoreCase(CatpMessage.BID));
					} else {
				...
				final ShoutReceivedEvent srEvent = new ShoutReceivedEvent(shout);
				final int time[] = Message.parseIntegers(request
						.getHeader(CatpMessage.TIME));
				srEvent.setTime(time);
				addEventDispatchingTask(srEvent);
	...
	@Override
	protected void postTransactionReceived(final Transaction transaction) {
	...
	/**
	 * The {@link edu.cuny.event.RichEvent}-based dispatching mechanism is used
	 ...
	 */
	@Override
	public void eventOccurred(final RichEvent event) {
