...
public class SlidingMatchedAverageBeatingAcceptingPolicy extends
		QuoteBeatingAcceptingPolicy {

	static Logger logger = Logger
			.getLogger(SlidingMatchedAverageBeatingAcceptingPolicy.class);
	...
	public void setup(final ParameterDatabase parameters, final Parameter base) {
		...
		learner.setup(parameters, base
				.push(SlidingMatchedAverageBeatingAcceptingPolicy.P_LEARNER));
		learner.initialize();
	}
	...
	protected void bidNotAnImprovementException() throws IllegalShoutException {
		if (SlidingMatchedAverageBeatingAcceptingPolicy.bidException == null) {
			// Only construct a new exception the once (for improved performance)
			SlidingMatchedAverageBeatingAcceptingPolicy.bidException = new IllegalShoutException(
					"Bid cannot beat the estimated equilibrium!");
		}
		throw SlidingMatchedAverageBeatingAcceptingPolicy.bidException;
	}

	protected void askNotAnImprovementException() throws IllegalShoutException {
		if (SlidingMatchedAverageBeatingAcceptingPolicy.askException == null) {
			// Only construct a new exception the once (for improved performance)
			SlidingMatchedAverageBeatingAcceptingPolicy.askException = new IllegalShoutException(
					"Ask cannot beat the estimated equilibrium!");
		}
		throw SlidingMatchedAverageBeatingAcceptingPolicy.askException;
	}
		...
		if (event instanceof TransactionExecutedEvent) {
			...
			if (learner.goodEnough()) {
				...
				ReportVariableBoard
						.getInstance()
						.reportValue(
								SlidingMatchedAverageBeatingAcceptingPolicy.SLIDING_AVERAGE_TRANSACTION_PRICE,
								learner.act());
			}
		}
	}
