...
import edu.cuny.config.param.ParamClassLoadException;
...
import edu.cuny.event.RichEvent;
...
public class GenericDoubleAuctioneer implements Serializable, Auctioneer,
	...
	/**
	 * helper components that are required and shared by policies to provide
	 * additional capabilities.
	 */
	protected Map<Class<? extends Helper>, Helper> helpers;
	...
	public void setup(final ParameterDatabase parameters, final Parameter base) {
		...
		try {
			shoutEngine = parameters.getInstanceForParameterEq(
					base.push(GenericDoubleAuctioneer.P_MATCHING),
					defBase.push(GenericDoubleAuctioneer.P_MATCHING), ShoutEngine.class);
		...
		if (shoutEngine instanceof Parameterizable) {
			((Parameterizable) shoutEngine).setup(parameters,
					base.push(GenericDoubleAuctioneer.P_MATCHING));
		}
		shoutEngine.initialize();

		try {
			quotingPolicy = parameters.getInstanceForParameterEq(
					base.push(GenericDoubleAuctioneer.P_QUOTING),
					defBase.push(GenericDoubleAuctioneer.P_QUOTING), QuotingPolicy.class);
		} catch (final ParamClassLoadException e) {
		...
		quotingPolicy.setup(parameters,
				base.push(GenericDoubleAuctioneer.P_QUOTING));
		quotingPolicy.initialize();

		pricingPolicy = parameters.getInstanceForParameterEq(
				base.push(GenericDoubleAuctioneer.P_PRICING),
				defBase.push(GenericDoubleAuctioneer.P_PRICING), PricingPolicy.class);
		pricingPolicy.setAuctioneer(this);
		pricingPolicy.setup(parameters,
				base.push(GenericDoubleAuctioneer.P_PRICING));
		pricingPolicy.initialize();

		clearingCondition = parameters.getInstanceForParameterEq(
				base.push(GenericDoubleAuctioneer.P_CLEARING),
				defBase.push(GenericDoubleAuctioneer.P_CLEARING),
				MarketClearingCondition.class);
		clearingCondition.setAuctioneer(this);
		clearingCondition.setup(parameters,
				base.push(GenericDoubleAuctioneer.P_CLEARING));
		clearingCondition.initialize();

		acceptingPolicy = parameters.getInstanceForParameterEq(
				base.push(GenericDoubleAuctioneer.P_ACCEPTING),
				defBase.push(GenericDoubleAuctioneer.P_ACCEPTING),
				ShoutAcceptingPolicy.class);
		acceptingPolicy.setAuctioneer(this);
		acceptingPolicy.setup(parameters,
				base.push(GenericDoubleAuctioneer.P_ACCEPTING));
		acceptingPolicy.initialize();

		chargingPolicy = parameters.getInstanceForParameterEq(
				base.push(GenericDoubleAuctioneer.P_CHARGING),
				defBase.push(GenericDoubleAuctioneer.P_CHARGING), ChargingPolicy.class);
		chargingPolicy.setAuctioneer(this);
		chargingPolicy.setup(parameters,
				base.push(GenericDoubleAuctioneer.P_CHARGING));
		chargingPolicy.initialize();

		try {
			subscribingPolicy = parameters.getInstanceForParameterEq(
					base.push(GenericDoubleAuctioneer.P_SUBSCRIBING),
					defBase.push(GenericDoubleAuctioneer.P_SUBSCRIBING),
		...
		subscribingPolicy.setup(parameters,
				base.push(GenericDoubleAuctioneer.P_SUBSCRIBING));
	...
	public void reset() {
		...
		for (AuctioneerPolicy policy : helpers.values()) {
			if (policy instanceof Resetable) {
				((Resetable) policy).reset();
			}
		}
	}
	...
	public void setSubscribingPolicy(final SubscribingPolicy subscribingPolicy) {
	...
	public void setRegistry(final MarketRegistry registry) {
		this.registry = registry;
	}

	/**
	 * 
	 * @param type the class of the requested {@link Helper}
	 * @return helper component of the given type
	 */
	@Override
	@SuppressWarnings("unchecked")
	public <H extends Helper> H getHelper(Class<H> type) {
		return (H) helpers.get(type);
	}

	/**
	 * 
	 * @param type the class of the {@link Helper}
	 * @param helper
	 */
	@Override
	public <H extends Helper> void setHelper(Class<H> type, H helper) {
	...
	public void newShout(final Shout shout) throws IllegalShoutException,
		...
		if (!shout.isValid()) {
			GenericDoubleAuctioneer.logger.error("malformed shout: " + shout);
			throw new IllegalShoutException("Malformed shout");
		} else if (acceptingPolicy == null) {
			GenericDoubleAuctioneer.logger.error("No accepting policy set up !");
	...
	protected void clear(final Shout ask, final Shout bid, final double price) {
		...
		final RichEvent event = new RichEvent(this, transaction);
