...
/**
 * charges only on profit. The fee on profit is 0 during the beginning certain
 * number of days, then set at a prefixed non-zero level for a certain number of
 * days. After another certain number of days, daily scores within a first
 * checking phase are collected, and then scores within a second phase with
 * higher profit fee are collected. After the two phases, depending upon whether
 * increasing fee in the second phase increases scores, the profit fee either
 * remain unchanged or is increased further.
 * 
...
public class MertacorChargingPolicy extends ChargingPolicy {
	...
	public static final int DEF_FIRST_INCREASE = 300;
	...
	public static final double DEF_STEP = 0.015;
	...
	public void setup(final ParameterDatabase parameters, final Parameter base) {
		...
		zeroDays = parameters.getIntWithDefault(base
				.push(MertacorChargingPolicy.P_ZERO_DAYS), defBase
				.push(MertacorChargingPolicy.P_ZERO_DAYS),
				MertacorChargingPolicy.DEFAULT_ZERO_DAYS);
		firstIncreaseDay = parameters.getIntWithDefault(base
				.push(MertacorChargingPolicy.P_FIRST_INCREASE), defBase
				.push(MertacorChargingPolicy.P_FIRST_INCREASE),
				MertacorChargingPolicy.DEF_FIRST_INCREASE);
		dayStep = parameters.getIntWithDefault(base
				.push(MertacorChargingPolicy.P_DAY_STEP), defBase
				.push(MertacorChargingPolicy.P_DAY_STEP),
				MertacorChargingPolicy.DEF_DAY_STEP);
		feeStep = parameters.getDoubleWithDefault(base
				.push(MertacorChargingPolicy.P_STEP), defBase
				.push(MertacorChargingPolicy.P_STEP), MertacorChargingPolicy.DEF_STEP);
	}
